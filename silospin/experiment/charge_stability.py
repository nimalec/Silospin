        ...
        if plot == True:
            ...
            def plot2Dtrace(i):
                ...
                if i == 0:
                    ...
                    cplot = ax.pcolor(V_x, V_y, V_out, cmap='RdBu', norm=plt.Normalize(z_min, z_max))
                    ax.set_xlabel("Left barrier voltage [V]")
                    ax.set_ylabel("Right barrier voltage [V]")
                    global v_out_temp
                    ...
                    cplot = ax.pcolor(V_x, V_y, V_out_temp, cmap='RdBu', norm=plt.Normalize(z_min, z_max))
                    ax.set_xlabel("Left barrier voltage [V]")
                    ax.set_ylabel("Right barrier voltage [V]")
                return cplot,
            plotter = FuncAnimation(fig, plot2Dtrace, frames=npoints[0]*npoints[1], interval=0.00001, repeat=False)
            return plotter, (V_x, V_y, V_out)
            plt.show()
