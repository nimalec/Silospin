...
def generate_reduced_command_table_rf_core_v1(n_pi_2, n_pi, n_p=[], arbZ=[]):
    ##Version adapted for RF in plunger frame
    #0-3: pi (pi_fr) [xx, yy, mxxm, myym]
    #4-7: pi/2 (pi/2_fr) [x, y, xxx, yyy]
    #8-11: pi/2 (pi_fr) [x, y, xxx, yyy]
    #12-18: pi (pi_fr) [0, 90, 180, 270, -90, -180, -270]
    ...
    ct = []
    initial_gates = {"xx_pi_fr": {"phi": 0, "wave_idx": 0}, "yy_pi_fr": {"phi": -90, "wave_idx": 0}, "mxxm_pi_fr": {"phi": -180, "wave_idx": 0}, "myym_pi_fr": {"phi": 90, "wave_idx": 0},  "x_pi_2_fr": {"phi": 0, "wave_idx": 1},  "y_pi_2_fr": {"phi": -90, "wave_idx": 1},  "xxx_pi_2_fr": {"phi": -180, "wave_idx": 1},  "yyy_pi_2_fr": {"phi": 90, "wave_idx": 1},  "x_pi_fr": {"phi": 0, "wave_idx": 2},  "y_pi_fr": {"phi": -90, "wave_idx": 2},  "xxx_pi_fr": {"phi": -180, "wave_idx": 2},  "yyy_pi_fr": {"phi": 90, "wave_idx": 2}}
    waves = [{"index": 0, "awgChannel0": ["sigout0","sigout1"]}, {"index": 1, "awgChannel0": ["sigout0","sigout1"]}, {"index": 2, "awgChannel0": ["sigout0","sigout1"]}, {"index": 3, "awgChannel0": ["sigout0","sigout1"]}, {"index": 4, "awgChannel0": ["sigout0","sigout1"]}, {"index": 5, "awgChannel0": ["sigout0","sigout1"]}, {"index": 6, "awgChannel0": ["sigout0","sigout1"]}]
    phases_0_I = [{"value": 0}, {"value": -90}, {"value": -180}, {"value": 90}]
    phases_0_Q = [{"value": -90}, {"value": -180}, {"value": -270}, {"value": 0}]
    phases_incr = [{"value": 0, "increment": True}, {"value": -90, "increment": True}, {"value": -180, "increment": True}, {"value": -270, "increment": True}, {"value": 90, "increment": True},  {"value": 180, "increment": True},{"value": 270, "increment": True}]
   ...
   ##Modifications: initial gates in DC frame
    #pi in p1 frame
    for i in range(len(phases_0_I)):
    ...
    #pi/2 in p1 frame
    for i in range(len(phases_0_I)):
          ct.append({"index": ct_idx, "waveform": waves[5], "phase0": phases_0_I[i], "phase1": phases_0_Q[i]})
          ct_idx += 1
    #pi/2 in p2 frame
    for i in range(len(phases_0_I)):
          ct.append({"index": ct_idx, "waveform": waves[6], "phase0": phases_0_I[i], "phase1": phases_0_Q[i]})
          ct_idx += 1
    ...
    ##Modification: Incremented RF in plunger frame
    ##pi in p1 frame
    for i in range(len(phases_incr)):
   ...
   ##pi in p2 frame
    ...
    ##pi/2 in p1 frame
    for i in range(len(phases_incr)):
          ct.append({"index": ct_idx, "waveform": waves[5], "phase0": phases_incr[i], "phase1": phases_incr[i]})
          ct_idx += 1
   ##pi/2 in p2 frame
    for i in range(len(phases_incr)):
          ct.append({"index": ct_idx, "waveform": waves[6], "phase0": phases_incr[i], "phase1": phases_incr[i]})
          ct_idx += 1
   ...
   #ct_idx = 80+n_p+n_z at this point
    command_table  = {'$schema': 'https://json-schema.org/draft-04/schema#', 'header': {'version': '1.0'}, 'table': ct}
    return command_table
...
def make_command_table_idxs_rf_p_v1(gt_seqs, taus_std, taus_p, n_arbZ):
    ##New version adapte for CT idxs with plunger gates being longer than RF gates
    arbZ = [] #output: list of arbZ tuples
    #ct_idxs: outer loop = 'rf' or 'plunger'. inner_loop = AWG cores.
    ct_idxs = {'rf': {}, 'plunger': {}} #command table indices
    #mapping of initial gate type to CT index
    #initial_gates = {'xx_pi_fr': 0, 'yy_pi_fr': 1, 'mxxm_pi_fr': 2, 'myym_pi_fr': 3, 'x_pi2_fr': 4, 'y_pi2_fr': 5, 'xxx_pi2_fr': 6, 'yyy_pi2_fr': 7, 'x_pi_fr': 8, 'y_pi_fr':  9, 'xxx_pi_fr':  10, 'yyy_pi_fr': 11}
    initial_gates = {'xx_pi_fr': 0, 'yy_pi_fr': 1, 'mxxm_pi_fr': 2, 'myym_pi_fr': 3, 'x_pi2_fr': 4, 'y_pi2_fr': 5, 'xxx_pi2_fr': 6, 'yyy_pi2_fr': 7, 'x_pi_fr': 8, 'y_pi_fr':  9, 'xxx_pi_fr':  10, 'yyy_pi_fr': 11, 'xx_p1_fr': 12, 'yy_p1_fr': 13, 'mxxm_p1_fr': 14, 'myym_p1_fr': 15, 'xx_p2_fr': 16, 'yy_p2_fr': 17, 'mxxm_p2_fr': 18, 'myym_p2_fr': 19,  'x_p1_fr': 20, 'y_p1_fr': 21, 'xxx_p1_fr': 22, 'yyy_p1_fr': 23, 'x_p2_fr': 24, 'y_p2_fr': 25, 'xxx_p2_fr': 26, 'yyy_p2_fr': 27}

    #mapping from  pi (pi_fr) incremental angle to ct indx
    ct_idx_incr_pi_pi_fr = {0: 28, -90: 29, -180: 30, -270: 31, 90: 32, 180: 33,  270: 34}
    #mapping from  pi_2 (pi_2_fr) incremental angle to ct indx
    ct_idx_incr_pi_2_pi_2_fr = {0: 35, -90: 36, -180: 37, -270: 38, 90: 39, 180: 40,  270: 41}
    #mapping from  pi_2 (pi_fr) incremental angle to ct indx
    ct_idx_incr_pi_2_pi_fr = {0: 42, -90: 43, -180: 44, -270: 45, 90: 46, 180: 47,  270: 48}

    #mapping from  pi (p1_fr) incremental angle to ct indx
    ct_idx_incr_pi_p1_fr = {0: 49, -90: 50, -180: 51, -270: 52, 90: 53, 180: 54,  270: 55}
    #mapping from  pi (p2_fr) incremental angle to ct indx
    ct_idx_incr_pi_p2_fr = {0: 56, -90: 57, -180: 58, -270: 59, 90: 60, 180: 61,  270: 62}

    #mapping from  pi/2 (p1_fr) incremental angle to ct indx
    ct_idx_incr_pi_2_p1_fr = {0: 63, -90: 64, -180: 65, -270: 66, 90: 67, 180: 68,  270: 69}
    #mapping from  pi/2 (p2_fr) incremental angle to ct indx
    ct_idx_incr_pi_2_p2_fr = {0: 70, -90: 71, -180: 72, -270: 73, 90: 74, 180: 75,  270: 76}

    #mapping from gate type to phase angle
    phi_ls_gt = {'x':  0, 'y': -90, 'xx':  0, 'yy': -90 , 'xxx':  -180, 'yyy': 90, 'mxxm': -180, 'myym': 90}
...
#     ## Extract mapping from taus_p to ct_idx
    ...
    ##'use_plunger_frame_rf' used to determine if plunger frame should be used in the prescence of this gate (0 if no plunger frame, 1 if plunger frame)
    use_plunger_frame_rf = {7: {'pi': 0, 'pi_2': 0} , 8: {'pi': 0, 'pi_2': 0}}
    taus_ct_idxs = {'rf': {'pi2': {'tau_pi2': taus_std[0], 'ct_idx': 77 }, 'pi': {'tau_pi': taus_std[1], 'ct_idx':  78}}, 'plunger': {}}
    idx = 1
    for item in taus_p:
        taus_ct_idxs['plunger'][item[0]] = {'tau_p': item[1], 'ct_idx': 78+idx}
        ##Append to use_plunger_frame_rf
        if item[1] > taus_std[1]:
            use_plunger_frame_rf[item[0]]['pi'] = 1
            use_plunger_frame_rf[item[0]]['pi_2'] = 1
        elif item[1] > taus_std[0]:
            use_plunger_frame_rf[item[0]]['pi_2'] = 1
        else:
            pass
        idx += 1
    ...
    ##Need to get intersection of RF with DC gates
    rf_ct_idxs = {}
    for rf_idx in rf_gate_sequence:
        #For a single AWG core ==> populate a list with CT idxs
        rf_ct_idxs[rf_idx] = []
        ...
        #Intersection of indices between differnet cores
        rf_diff_idxs = list(set([i for i in rf_gate_sequence.keys()]).difference(rf_idx))
        p_diff_idxs = list([i for i in plunger_gate_sequence.keys()])
        ...
        for idx in range(n_gates):
            ...
            ##Case A: Initial gates of sequence
            if idx == 0:
                ##Need to add additional case here  ==> when plunger > pi
               # 1. Case 1: xx, yy, mxxm, myym [pi (pi_fr)]
                if gt in pi_gt_set:
                    ## 1. Check if there's an intersection with plunger gates.
                    ## 2. If there's an intersection and p1 or or p2 has a length greater than the pulse length,
                    ## use (pi)_p1 or (pi)_p2 [maybe make a flag for pi_std < p1 or p2]...
                    ## 3. If there's no intersection, use the std command table index
                    gt_str = gt+'_pi_fr'
                    rf_ct_idx_list.append(initial_gates[gt_str])
                ...
                #2. Case 2: pi/2 gates
                    ...
                    ## 1. Check if there's an intersection with plunger gates.
                    ## 2. If there's an intersection and p1 or or p2 has a length greater than the pulse length,
                    ## use (pi/2)_p1 or (pi/2)_p2 [maybe make a flag for pi_std < p1 or p2]...
                    ## 3. If there's no intersection, use the std command table index.
                    if len(pi_intersect)>0:
                        ##work in pi frame
                        gt_str = gt+'_pi_fr'
                ...
                #3. Case 3: wait is present
                ## Need to change indices for waits...
                elif gt[0] == 't':
                    #Gets duration for single gate
                    gt_t_str = int(gt[1:len(gt)])
                    #Checks if duration corresponds to pi/2 pulse duration
                    if gt_t_str == taus_std[0]:
                        ##assign ct index 33 if pi/2
