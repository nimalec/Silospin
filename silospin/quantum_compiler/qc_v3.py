...
from silospin.quantum_compiler.qc_helpers_v3 import *
    ...
    def __init__(self, gst_file_path, awg, gate_parameters, n_inner=1, n_outer=1, external_trigger=True, trigger_channel=0, sample_rate=2.4e9):
       ...
       ##assumes that plungers will alwyas be shorter than rf pulses
        ...
        for idx in self._gate_sequences:
            ...
            ct_idxs_all[idx], arbZ = make_command_table_idxs_rf_p_v1(gate_sequence, taus_std_v2, plunger_set, n_arbZ)
            n_arbZ += len(arbZ)
            arbZs.append(arbZ)
        ...
        command_table_plunger = generate_reduced_command_table_p_core_v2(n_p=plunger_set_npoints_tups, n_rf=(npoints_pi_2_standard, npoints_pi_standard))
...
class GateSetTomographyProgramPlunger_V4:
    """
    Class representing an instance of compiler gate set tomography experiment (uses entirely rectangular waves).
    ..
    Attributes
    ...
    def __init__(self, gst_file_path, awg, gate_parameters, n_inner=1, n_outer=1, external_trigger=True, trigger_channel=0, sample_rate=2.4e9, added_padding=0):
       ...
       ##assumes that plungers will alwyas be shorter than rf pulses
        ...
        n_waveform_pi_2_std  = len(self._waveforms[1]["pi_pi_2fr"])
        ...
        command_tables_rf = generate_reduced_command_table_rf_core_v0(n_waveform_pi_2_std, n_waveform_pi_std, n_p=plunger_set_npoints, arbZ=arbZ_s)
        command_table_plunger = generate_reduced_command_table_p_core_v2(n_p=plunger_set_npoints_tups, n_rf=(n_waveform_pi_2_std,  n_waveform_pi_std))
        self._ct_idxs = ct_idxs_all
        self._command_tables = {'rf': command_tables_rf, 'plunger': command_table_plunger}
