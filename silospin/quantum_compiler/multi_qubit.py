...
class MultiQubitGST:
    def __init__(self, gst_file_path, awg, qubits = {1,2,3,4}):
        ...
        for idx in self._gate_sequences:
            ...
            for key in gate_sequence: #loops over qubits
                ...
                if gt_seq:
                    ...
                    cts_idxs[key] = ct_idxs
                    n_t_ts[key] = n_t_ts[key] + len(n_ts)
                else:
                    pass
        ...
        for i in self._awg_cores:
            command_table = generate_reduced_command_table(gts_0, npoints_wait = n_waits, npoints_plunger = None, delta_iq = self._qubit_parameters[str(i)]["delta_iq"], phi_z = 0)
