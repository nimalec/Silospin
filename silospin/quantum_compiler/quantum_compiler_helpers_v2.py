...
def make_command_table_indices(gt_seqs, taus_std, taus_p, n_arbZ):
    ...
    for rf_idx in rf_gate_sequence:
        ...
        if gt_0 in {'x', 'y', 'xxx', 'yyy', 'mxxxm', 'myyym' 'xx', 'yy'}:
...
def make_command_table_indices_v3(gt_seqs, channel_map, awg_core_split, arb_gates, plunger_tup_lengths, taus_std, gate_lengths, arbgate_counter, arbZs, pickle_file_location='C:\\Users\\Sigillito Lab\\Desktop\\experimental_workspaces\\quantum_dot_workspace_bluefors1\\experiment_parameters\\bluefors1_arb_gates.pickle'):
    ## Should return
    ##Modifications here: 1. accomodate for multiple cores/channels, 2. arb Z should be counted and account for each edge case,
    ## 3. arbitrary gates ==> need to add new command table index when these occur (per core).
    ## Should alo return a list of command table indices corresponding to each arbitrary gate.
...
def make_command_table_indices_v4(gt_seqs, channel_map, awg_core_split, arb_gates, plunger_tup_lengths, taus_std, gate_lengths, arbgate_counter, arbZs, line, arb_dc_dict, pickle_file_location='C:\\Users\\Sigillito Lab\\Desktop\\experimental_workspaces\\quantum_dot_workspace_bluefors1\\experiment_parameters\\bluefors1_arb_gates.pickle'):
    ## Should return
    ##Modifications here: 1. accomodate for multiple cores/channels, 2. arb Z should be counted and account for each edge case,
    ## 3. arbitrary gates ==> need to add new command table index when these occur (per core).
    ## Should alo return a list of command table indices corresponding to each arbitrary gate.
...
def make_rf_command_table_v2(n_std, arbZs, arbitrary_waveforms, plunger_length_set, awgidx, coreidx):
    ...
    for awg in channel_map:
        ...
        for core_idx in channel_map[awg]:
            ...
            if channel_map[awg][core_idx]['rf'] == 1:
                ...
                ch_1_idx = channel_map[awg][core_idx]['gate_idx'][0]
                ch_2_idx = channel_map[awg][core_idx]['gate_idx'][1]
                ## Note: a condition added when plunger size  > number of points
        ...
        for core_idx in channel_map[awg]:
            ...
            if channel_map[awg][core_idx]['rf'] == 1:
                ...
                for ch_idx in plunger_channels:
                    ...
                    waveforms[awg][core_idx]["p"+str(ch_1_idx)+"_p"+str(ch_idx)+"fr"] = rectangular_add_padding(gate_npoints[awg]["plunger"][ch_1_idx]["p"], amp, min_points = frame_p1_points, side_pad=added_padding)
                    waveforms[awg][core_idx]["p"+str(ch_2_idx)+"_p"+str(ch_idx)+"fr"] = rectangular_add_padding(gate_npoints[awg]["plunger"][ch_2_idx]["p"], amp, min_points = frame_p2_points, side_pad=added_padding)
                    print("p"+str(ch_1_idx)+"_p"+str(ch_idx)+"fr",ch_1_idx, gate_npoints[awg]["plunger"][ch_1_idx]["p"],frame_p1_points, "p"+str(ch_2_idx)+"_p"+str(ch_idx)+"fr", ch_2_idx, gate_npoints[awg]["plunger"][ch_2_idx]["p"],frame_p2_points)
                    print(len(waveforms[awg][core_idx]["p"+str(ch_1_idx)+"_p"+str(ch_idx)+"fr"]), len(waveforms[awg][core_idx]["p"+str(ch_1_idx)+"_p"+str(ch_idx)+"fr"]))
